...
def process_frame(frame, lgo, logo_position, move_up):
    ...
    frame = cv2.addWeighted(ovr, 0.5, frame, 1.0, 0, frame)
...
def process_video(video_path, output_path, audio_output_path):
    ...
    global lgo
    lgo = cv2.cvtColor(lgo_img, cv2.COLOR_BGRA2BGR)  # Dört kanallı görüntüyü üç kanallıya dönüştür
    ...
    print(f"Video {os.path.basename(video_path)} success.")

if __name__ == "__main__":
    input_folder = 'videos'
    output_folder = 'proccessedvideos'
